use wgpu::{util::{BufferInitDescriptor, DeviceExt}, BindGroup, Buffer, CommandEncoderDescriptor, Device, Queue, RenderPipeline, Texture, TextureViewDescriptor};
...
impl Identity {
    ...
    fn create_vertex_buffer(device: &wgpu::Device) -> (Buffer, wgpu::VertexBufferLayout) {
    ...
    fn create_bind_group(
        device: &wgpu::Device, sampler: &wgpu::Sampler, texture: &Texture
    ) -> (BindGroup, wgpu::BindGroupLayout) {
    ...
    fn create_pipeline(
        device: &wgpu::Device, bind_group_layouts: &[&wgpu::BindGroupLayout],
        vertex_buffer_layout: wgpu::VertexBufferLayout
    ) -> RenderPipeline {
        let render_shader = device.create_shader_module(
            wgpu::ShaderModuleDescriptor {
                label: Some("Gray Render Shader"),
                source: wgpu::ShaderSource::Wgsl(include_str!("shaders/identity.wgsl").into())
            }); 
